...
(function() {
  ...
  var wordStartPosition;  // Denotes the current starting point of the current word
  ...
  function activate(lang, state, options) {
    ...
    wordStartPosition = state.selectionStart;
  ...
  function setLanguage(newlang) {
    ...
    getDictionaryDataPromise
      .then(function(values) {
        ...
        if (!builtInDict) {
          console.error(
            'thai.js: dictionary is specified but can\'t be loaded.');
          language = undefined;
  ...
  function setLanguageSync(newlang, dictData, userDict) {
    ...
    if (!worker) {
      ...
      worker = new Worker('js/imes/thai/worker.js');
  ...
  function handleKey(keycode, repeat) {
    ...
    return keyboard.sendKey(keycode, repeat).then(function() {
      ...
      if (keycode === BACKSPACE) {
        ...
        // If the cursor gets before word start position, set it as the new word start position
        if(cursor < wordStartPosition){
  ...
  function updateSuggestions(repeat) {
    ...
    if (!worker) {
      console.error('thai.js: called updateSuggestions() w/o an worker.');
  ...
  function wordBeforeCursor() {
    ...
    */
    ...
    return inputText.substring(wordStartPosition, cursor);
  }
