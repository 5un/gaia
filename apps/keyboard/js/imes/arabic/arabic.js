...
/*
 * This Arabic input method provides four forms of input assistance:
 *
 * 1) word suggestions
 *
 * 2) auto correction
...
(function() {
  ...
  var USER_DICT_DB_NAME = 'UserDictArabic';
  ...
  function setLanguage(newlang) {
    ...
    getDictionaryDataPromise
      .then(function(values) {
        ...
        if (!builtInDict) {
          console.error(
            'arabic.js: dictionary is specified but can\'t be loaded.');
          language = undefined;
  ...
  function setLanguageSync(newlang, dictData, userDict) {
    ...
    if (!worker) {
      ...
      worker = new Worker('js/imes/arabic/worker.js');
  ...
  function updateSuggestions(repeat) {
    ...
    if (!worker) {
      console.error('arabic.js: called updateSuggestions() w/o an worker.');
